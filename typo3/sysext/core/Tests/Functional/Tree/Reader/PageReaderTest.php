...
class PageReaderTest extends \TYPO3\CMS\Core\Tests\FunctionalTestCase
{
     ...
     * @param int $depth
     * @param array $expandedIdentifiers
     * @param array $expected
     *
     * @test
     * @dataProvider getReturnsValidNodesDataProvider
     */
    public function getReturnsValidNodes($backendUserId, $identifier, $depth, array $expandedIdentifiers, $expected)
    {
        $backendUser = $this->setUpBackendUser($backendUserId);
        $backendUser->uc['BackendComponents']['States']['Pagetree']->stateHash = new \stdClass();

        if ($expandedIdentifiers === ['*']) {
            $this->subject->setExpandAll(true);
        } else {
            foreach ($expandedIdentifiers as $expandedIdentifier) {
                $expandedIdentifierHex = dechex($expandedIdentifier);
                $backendUser->uc['BackendComponents']['States']['Pagetree']->stateHash->{$expandedIdentifierHex} = true;
            }
        }
